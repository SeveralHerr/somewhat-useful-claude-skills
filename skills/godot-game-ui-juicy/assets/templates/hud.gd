...
func add_stat(id: StringName, glyph: UiTheme.Glyph.Kind, initial: String = "0") -> void:
	...
	# punched, and flash_stat's punch would be dead code that nobody notices because the tint
	# still lands. The shell is what the VBox lays out; the row inside it moves freely.
...
##
## Colour AND squash: the tint says which kind of event it was, the punch says an event
## happened at all. Peripheral vision picks up the movement long before it resolves the
## colour, which is why a stat pill that only changes colour so often goes unnoticed.
func flash_stat(id: StringName, color: Color) -> void:
	...
	UiJuice.punch(self, entry["row"] as Control, 0.1)
...
func toast(text: String, color: Color = UiTheme.ACCENT) -> void:
	...
	# Scale has no such problem — pop_in returns it to the same resting scale every time.
...
func build_tools(icons: Array[UiTheme.Glyph.Kind]) -> void:
	...
	for i: int in range(icons.size()):
		...
		# cannot be scaled, so selecting a tool would tint it and nothing more.
...
func set_active_tool(index: int) -> void:
	...
	if index >= 0 and index < _tool_slots.size():
		UiJuice.punch(self, _tool_slots[index], 0.18)


## Shake the whole HUD. For damage, a failed action, anything the player should feel rather
## than read. Keep it under a fifth of a second — a shake that outlasts its cause reads as
## a bug, and shaking the HUD when the *world* should shake is the most common misuse.
func shake(strength: float = 9.0) -> void:
	_ensure_built()
	UiJuice.shake(self, _root, strength)


## Full-screen colour pulse: red for damage, white for a pickup, gold for a milestone.
func flash_screen(color: Color = Color(1, 1, 1, 0.4), time: float = 0.22) -> void:
	_ensure_built()
	UiJuice.flash(self, color, time)
...
func _build_bottom() -> void:
	...
	# The counter is the element most worth punching — it is the number the player is watching
	# — and `bar` is an HBoxContainer, so it needs the same shell. The shell takes the layout
	# flags the label used to carry, since the shell is now what the bar lays out.
	# The shell hangs at the END of the bar rather than expanding: the toolbar already takes
	# all the slack, so shrink-end puts the counter hard against the right margin and a number
	# that grows a digit extends leftward instead of shifting the whole readout.
